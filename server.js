...
app.get("/api/buscar-atacado", async (req, res) => {
  try {
    ...
    let lista = (d.data || []).map(p => ({ id: p.id, nome: p.nome, codigo: p.codigo, estoque: p.estoque?.saldoVirtualTotal ?? null }));
    const f = lista.filter(p => (p.nome || "").toLowerCase().includes(termo));
    lista = f.length ? f : lista;
    // aplica o preço de atacado a partir da tabela publicada (por código)
    ...
    lista.forEach(p => { p.precoAtacado = precoPorCod[String(p.codigo)] ?? null; });
    res.json({ data: lista });
  } catch (e) { res.status(e.status || 500).json({ erro: e.message, body: e.body }); }
});
