...
app.post("/api/nfce/pedido/:id/emitir",async(req,res)=>{
  try{
    ...
    // se pediu pra emitir em nome de "Consumidor Final": troca o contato SÓ NA
    // NOTA (rascunho), nunca no pedido de venda -- são coisas distintas, o
    // pedido continua com o cliente de verdade. Usa PUT /nfce/{id} (existe na
    // API v3 pra editar a nota antes de enviar pra SEFAZ), pegando o rascunho
    // inteiro e só mudando o contato, pra não perder nenhum campo obrigatório
    // que o Bling já preencheu sozinho.
...
// (o diagnóstico antigo de situações foi removido: ele registrava a MESMA rota
// /api/diag/situacoes e vencia a nova, então a busca corrigida nunca era usada)
...
app.get("/api/diag/rota-pedido/:numero",async(req,res)=>{
  try{
    ...
    // filtra direto no Bling por numero= (descoberto 26/09 -- antes varria até 30
    // páginas achando na unha, sem necessidade)
    let achado=null;
    try{
      const arrF=await bling(`/pedidos/vendas?numero=${encodeURIComponent(num)}&limite=10`).then(r=>r?.data||[]);
      achado=arrF.find(x=>String(x.numero)===num)||null;
    }catch(e){}
    if(!achado){
...
async function moverPedidoParaAtendido(pedidoId, opts={}){
  ...
  // pedido vem de PRAZO ou já está EM ROTA. Um pedido em PRAZO já teve a mercadoria
  // entregue e a baixa de estoque feita há muito tempo (não passou pela separação
  // agora); um pedido EM ROTA já passou por Separado/Conferência antes de sair pra
  // entrega (é exigido lá na frente) — forçar SEPARADO de novo nesses casos tentava
  // mexer numa etapa que já não existe mais, e podia deixar o pedido travado em vez
  // de virar ATENDIDO quando o Bling recusava (ex.: nova tentativa de baixa de
  // estoque já consumida, ou transição sem sentido Em Rota → Separado).
  if(sitAtual!==SIT.SEPARADO && sitAtual!==SIT.PRAZO && sitAtual!==SIT.EM_ROTA){
